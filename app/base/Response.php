...
trait Response
{
     ...
     * Description 返回消息信息
     */
    protected static function returnMsg($data): Think_Response
    {
        if (self::$msgType == 'ajax') {
            return Think_Response::create(self::getAjaxReturn($data), 'Json', 200)->contentType('application/json');
        } else if (self::$msgType == 'xml') {
            return Think_Response::create(self::getXmlReturn($data), 'xml', 200)->contentType('application/xml');
        } else {
            return Think_Response::create(self::getAjaxReturn($data), 'Json', 200)->contentType('application/json');
        }
    }
