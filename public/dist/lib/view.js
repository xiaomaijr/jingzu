...
layui.define(['laytpl', 'layer'], function (exports) {
    ...
    view.request = function (options) {
        if (options.auth) {
            if (options.data) {
                options.data.access_token = layui.data(setter.tableName)[setter.request.tokenName]
            } else {
                options.data = {access_token: layui.data(setter.tableName)[setter.request.tokenName]}
            }
        }
        ...
        if (options.url.slice(0, 2) !== './') {
            options.url = setter.api_host + options.url;
        }
        //发送ajax请求
        $.ajax({
            url: options.url,
            ...
            beforeSend: function () {
                ...
                if (options.beforeSend !== undefined && options) {
                    options.beforeSend()
                }
            }, success: function (e) {
                if (options.url.slice(0, 2) === './') {
                    ...
                    if (options.tips) {
                        if (data.success) {
                            var msg_config = {time: 2000, offset: '15px', icon: 1}
                        } else {
                            var msg_config = {time: 2000, offset: '15px', icon: 2}
                        }
                ...
                layer.close(loading);
                layer.msg('接口请求错误,请联系管理员', {time: 3000, offset: '15px', icon: 2})
            }
        })
    };

    view.getTableHeight = function (n) {
        var remove = n === undefined ? 0 : n;
        return $('.layui-body').height() - $('.layui-breadcrumb').height() - 20 - remove;
    }
    //请求模板文件渲染
    ...
    view.cutString = function (str, len) {
        //length属性读出来的汉字长度为1
        if (str.length * 2 <= len) {
            return str;
        }
